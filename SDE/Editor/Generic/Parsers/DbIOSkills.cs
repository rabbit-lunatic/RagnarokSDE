    ...
    public sealed class DbIOSkills
    {
            ...
            if (debug.FileType == FileType.Yaml)
            {
                ...
                foreach (var skill in ele.Output["copy_paste"] ?? ele.Output["Body"])
                {
                    try
                    {
                        ...
                        // Add GiveAp
                        table.SetRaw(skillId, ServerSkillAttributes.GiveAp, _loadListToString(skill["GiveAp"], "Level", "Amount"));
                        ...
                        if (skill["Requires"] != null)
                        {
                            ...
                            //Add ApCost
                            table.SetRaw(skillId, ServerSkillAttributes.RequireApCost, _loadListToString(entry["ApCost"], "Level", "Amount"));
                            ...
                            //Add ApRateCost
                            table.SetRaw(skillId, ServerSkillAttributes.RequireApRateCost, _loadListToString(entry["ApRateCost"], "Level", "Amount"));
                            ...
                            table.SetRaw(skillId, ServerSkillAttributes.RequireWeapons, DbIOUtils.LoadFlag<SkillRequireWeaponType>(entry["Weapon"], "0xFFFFFF"));
                        ...
                        // Add Status
                        table.SetRaw(skillId, ServerSkillAttributes.Status, skill["Status"] ?? "");
                    }
        ...
        public static void WriteEntryYaml(StringBuilder builder, ReadableTuple<int> tuple, MetaTable<int> itemDb)
        {
            if (tuple != null)
            {
                ...
                //Add GiveAp
                _expandList(builder, tuple, "GiveAp", "      - Level", "Amount", ServerSkillAttributes.GiveAp, YamlParser.Indent4, YamlParser.Indent8, "0", itemDb);
                ...
                // Add ApCost
                _expandList(require, tuple, "ApCost", "        - Level", "Amount", ServerSkillAttributes.RequireApCost, YamlParser.Indent6, YamlParser.Indent10, "0", itemDb);
                ...
                // Add ApRateCost
                _expandList(require, tuple, "ApRateCost", "        - Level", "Amount", ServerSkillAttributes.RequireApRateCost, YamlParser.Indent6, YamlParser.Indent10, "0", itemDb);
                ...
                if (value != 0xFFFFFF)
                {
                    DbIOUtils.ExpandFlag<SkillRequireWeaponType>(require, tuple, "Weapon", ServerSkillAttributes.RequireWeapons, YamlParser.Indent6, YamlParser.Indent8);
                }
                ...
                // Add Status
                if ((valueS = tuple.GetValue<string>(ServerSkillAttributes.Status)) != "")
                {
